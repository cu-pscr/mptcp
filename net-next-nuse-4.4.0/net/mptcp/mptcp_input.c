...
u64 start_time=0;
u64 start_bytes=0;
u64 sub_start_bytes=0;
u64 s_start_bytes=0;
u64 l_start_bytes=0;
u64 s_prev_bytes=0;
u64 l_prev_bytes=0;
u64 sub_prev_bytes=0;
u64 prev_time=0;
u64 prev_bytes=0;
long int mthr=0;
long int emthr=0;
...
u32 ofo_count=0;
u32 sandy_OFO_len=0;
void mptcp_queue_probe_log_hook(struct sock *sk,u8 q_id, struct tcp_sock *meta_tp, struct sk_buff *skb, u8 op_id) {
	ktime_t cur_time=ktime_get();
	struct sock *meta_sk = mptcp_meta_sk(sk);
        u32 cnt=ofo_length;
	struct timespec tv= ktime_to_timespec(ktime_sub(cur_time, sandy_time));
	struct sock *sk_it=NULL;
	struct tcp_sock *sub_tp=NULL;
	u32 hs_rtt=0,OFO_len=0,skb_OFO_len=0,RCV_len=0,skb_RCV_len=0,ls_bytes_received=0xffffffff,p1_rtt=0,p2_rtt=0,hs_bytes_received=0,hs_advmss=0,preque_len=0;
	struct sk_buff *tmp=NULL,*tmp1=NULL;
        switch (op_id) {
	...
	//ofo_count+=1;
        ofo_length = cnt;
	if(q_id==1 && meta_tp!=NULL && skb!=NULL){
		OFO_len=skb_queue_len(&meta_tp->out_of_order_queue);
		if(p_ofo==0)
			p_ofo=OFO_len;
		if(OFO_len>p_ofo)
			total_ofo+=(OFO_len-p_ofo);
		else
			if(OFO_len!=0)
				total_ofo-=(p_ofo-OFO_len);
			else{
				ofo_c+=1;
			}
		if(op_id==1)
			//printk("OFO is decremented by %u and length now: %u highest OFO %u sum OFO %u count %u and receive queue %u\n",(TCP_SKB_CB(skb)->end_seq - TCP_SKB_CB(skb)->seq),skb_queue_len(&meta_tp->out_of_order_queue),h_ofo,total_ofo,ofo_c,skb_queue_len(&meta_sk->sk_receive_queue));
			printk("\nOFO decremented length *** %u *** %u ***by %u len=%u\n",sandy_OFO_len,skb_queue_len(&meta_tp->out_of_order_queue),(TCP_SKB_CB(skb)->end_seq - TCP_SKB_CB(skb)->seq),skb->len);
		else if(op_id==0)
			//printk("OFO is incremented by %u and length now: %u highest OFO %u sum OFO %u count %u and receive queue %u \n",(TCP_SKB_CB(skb)->end_seq - TCP_SKB_CB(skb)->seq),skb_queue_len(&meta_tp->out_of_order_queue),h_ofo,total_ofo,ofo_c,skb_queue_len(&meta_sk->sk_receive_queue));
			printk("\nOFO incremented length *** %u *** %u ***by %u len=%u\n",sandy_OFO_len,skb_queue_len(&meta_tp->out_of_order_queue),(TCP_SKB_CB(skb)->end_seq - TCP_SKB_CB(skb)->seq),skb->len);
		else
			printk("\nOFO set to zero *** %u *** %u ***by %u len=%u \n",sandy_OFO_len,skb_queue_len(&meta_tp->out_of_order_queue),(TCP_SKB_CB(skb)->end_seq - TCP_SKB_CB(skb)->seq),skb->len);
		if(skb_queue_len(&meta_tp->out_of_order_queue)>h_ofo)
			h_ofo=skb_queue_len(&meta_tp->out_of_order_queue);
		p_ofo=OFO_len;
	}
	/*if(meta_sk!=NULL && meta_tp!=NULL){
		printk("sandy:The OFO queue length is %u  %llu receiver buffer %d   receiver window  %d receiver space %d and receiver rmem_alloc %d\n",ofo_length,ofo_count,meta_sk->sk_rcvbuf,meta_tp->rcv_wnd,tcp_space(meta_sk),meta_sk->sk_rmem_alloc);
	}*/
	...
	//Set the flag for the higher RTT tcp subflow
        mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
		sub_tp=tcp_sk(sk_it);
		if((sub_tp->srtt_us>>3)>hs_rtt)
			hs_rtt=(sub_tp->srtt_us>>3);
       }


       // Set flag for lowers bytes_received
	mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                sub_tp=tcp_sk(sk_it);
                if((sub_tp->bytes_received<ls_bytes_received)){
			//printk("Bytes received %llu\n",sub_tp->bytes_received);
                        ls_bytes_received=sub_tp->bytes_received;
		}
       }
	...
	mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                sub_tp=tcp_sk(sk_it);
                if((sub_tp->bytes_received>hs_bytes_received)){
                        //printk("Bytes received %llu\n",sub_tp->bytes_received);
                        hs_bytes_received=sub_tp->bytes_received;
                }
       }	

       mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                sub_tp=tcp_sk(sk_it);
                if((sub_tp->advmss>hs_advmss)){
                        //printk("Bytes received %llu\n",sub_tp->bytes_received);
                        hs_advmss=sub_tp->advmss;
                }
       }
       ...
       mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                sub_tp=tcp_sk(sk_it);
		if(!p1_rtt)
			p1_rtt=sub_tp->srtt_us>>3;
		else{
			if(!p2_rtt)
				p2_rtt=sub_tp->srtt_us>>3;
		}
       }

	mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                sub_tp=tcp_sk(sk_it);
		//printk("\nSub flow receive queue length %u and OFO length %u\n",skb_queue_len(&sk_it->sk_receive_queue),skb_queue_len(&sub_tp->out_of_order_queue));
       }

       OFO_len=skb_queue_len(&meta_tp->out_of_order_queue);
       RCV_len=skb_queue_len(&meta_sk->sk_receive_queue);
       preque_len=skb_queue_len(&meta_tp->ucopy.prequeue);*/
	
/*	
       skb_queue_walk_safe(&meta_tp->out_of_order_queue, tmp1, tmp) {
		skb_OFO_len+=tmp->data_len;
	}
//	printk("Total receive queu %u\n",RCV_len);
	skb_queue_walk_safe(&meta_sk->sk_receive_queue, tmp1, tmp) {

		//printk("Receive sk buffer data lenght %u\n",tmp->data_len);
                skb_RCV_len+=tmp->data_len;
        }
  */    
       /*if(sysctl_mptcp_sandy_mptcp!=0){

	       if(OFO_len>(RCV_len) && RCV_len>1  && p1_rtt!=p2_rtt){
	       //if(OFO_len>(RCV_len) && RCV_len>1 &&OFO_len>=(hs_bytes_received/hs_advmss)  && p1_rtt!=p2_rtt){
			//If RTT is different
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
		        	sub_tp=tcp_sk(sk_it);
		        	//if((sub_tp->srtt_us>>3)==hs_rtt && sub_tp->bytes_received<=ls_bytes_received  && (tcp_time_stamp - sub_tp->sandy_block_time > usecs_to_jiffies(sub_tp->srtt_us >> 3))){
		        	if((sub_tp->srtt_us>>3)==hs_rtt && (tcp_time_stamp - sub_tp->sandy_block_time > usecs_to_jiffies(sub_tp->srtt_us >> 3))){
					printk("OFO queue lenght %u is greater than the receiver queue %u and highest RTT  %u and least Bytes received %llu largest bytes %llu and MSS %u MPTCP rcv_wnd %u\n",OFO_len,RCV_len,sub_tp->srtt_us>>3,ls_bytes_received,hs_bytes_received,hs_advmss,meta_tp->rcv_wnd);
	    	                    	//sub_tp->sandy_block=1;
					sub_tp->sandy_block_time=tcp_time_stamp;				
					sub_tp->sandy_alpha+=2;
				}
	       		}
		}
		else if(OFO_len>(RCV_len) && RCV_len>1 && p1_rtt==p2_rtt ){
		//else if(OFO_len>(RCV_len) && RCV_len>1 && p1_rtt==p2_rtt && OFO_len>=(hs_bytes_received/hs_advmss)){
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
				sub_tp=tcp_sk(sk_it);
				if(ls_bytes_received==sub_tp->bytes_received && (tcp_time_stamp - sub_tp->sandy_block_time > usecs_to_jiffies(sub_tp->srtt_us >> 3))){
					printk("OFO queue lenght %u is greater than the receiver queue %u and lowest bytes_received %llu largest bytes %llu and MSS %u MPTCP rcv_wnd\n",OFO_len,RCV_len,ls_bytes_received,hs_bytes_received,hs_advmss,meta_tp->rcv_wnd);
		                        //sub_tp->sandy_block=1;
		                        sub_tp->sandy_block_time=tcp_time_stamp;
					sub_tp->sandy_alpha+=2;
				}
			...
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
				sub_tp=tcp_sk(sk_it);
				if(sub_tp->sandy_block==1 && ((tcp_time_stamp - sub_tp->sandy_block_time) > (usecs_to_jiffies(sub_tp->srtt_us >> 3)<<sub_tp->sandy_alpha))){
					printk("OFO queue is cleared we will enable back the path and alpha %llu\n",sub_tp->sandy_alpha);
					sub_tp->sandy_block=0;
					tcp_send_ack(sk_it);			
					sub_tp->sandy_alpha=1;
				}
			}
		}
       }*/
...
static int mptcp_detect_mapping(struct sock *sk, struct sk_buff *skb)
{
	...
	if (!data_len) {
		...
		mptcp_queue_probe_log_hook(sk,1, meta_tp, NULL, 2);
...
static int mptcp_queue_skb(struct sock *sk)
{
	 ...
	 */
	//mptcp_queue_probe_log_hook(sk,4, meta_tp, NULL, 4);
	...
	if (before64(rcv_nxt64, tp->mptcp->map_data_seq)) {
		...
		skb_queue_walk_safe(&sk->sk_receive_queue, tmp1, tmp) {
			...
			if (!mpcb->in_time_wait){ /* In time-wait, do not receive data */
				mptcp_queue_probe_log_hook(sk,1, meta_tp, tmp1, 0);
		...
		skb_queue_walk_safe(&sk->sk_receive_queue, tmp1, tmp) {
			...
			if (!eaten){
				mptcp_queue_probe_log_hook(sk,0, meta_tp, tmp1, 0);
...
void mptcp_data_ready(struct sock *sk)
{
	...
	struct tcp_sock *meta_tp=tcp_sk(meta_sk);	
	struct tcp_sock *sub_tp=NULL;
	long int total_download=0,sub_total_download=0,s_total_download=0,l_total_download=0;
	long int total_time=0;
	long int subtotal=0,s_total=0,l_total=0;
	u32 s_rtt=0xffffffff,l_rtt=0;
	bool condition;
	long int tbest=0; 
	int path=0;
	int ofo_diff=0;
	int ratio=0;
	struct tcp_sock *s_tp=NULL,*l_tp=NULL;
	struct sock *s_sk=NULL,*l_sk=NULL;
	int rttdiff=0;
...
restart:
	...
	skb_queue_walk_safe(&sk->sk_receive_queue, skb, tmp) {
		...
		/* Push a level higher */
		//sandy: Lets get the OFO and received bytes
		if(meta_tp && meta_sk){
			s_tp=NULL;
			l_tp=NULL;	
			subtotal=0;
                	//printk("\nMPTCP level bytes received %llu and OFO %u inorder queue length %u\n",meta_tp->bytes_received,skb_queue_len(&meta_tp->out_of_order_queue),skb_queue_len(&meta_sk->sk_receive_queue));
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                		sub_tp=tcp_sk(sk_it);
				sub_tp->sandy_best=0;
				subtotal+=sub_tp->bytes_received;		
				//if(sub_tp->sandy_astime!=0)
					//sub_tp->sandy_rtt=jiffies_to_msecs(sub_tp->rcv_tstamp)-sub_tp->sandy_astime;
       			}
			
			//find the smallest RTT path:
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                                sub_tp=tcp_sk(sk_it);
                                if((sub_tp->srtt_us>>3)<s_rtt)
                                //if((sub_tp->sandy_rtt)<s_rtt)
                        		//s_rtt=(sub_tp->sandy_rtt);
					s_rtt=sub_tp->srtt_us>>3;

                        }

			//Get the socket and tp socket for each path
			path=0;
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                                sub_tp=tcp_sk(sk_it);
				if((sub_tp->srtt_us>>3)==s_rtt && !path){
					s_tp=sub_tp;
					s_sk=sk_it;
                                        path=1;
				}
			...
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                                sub_tp=tcp_sk(sk_it);
                                //if((sub_tp->sandy_rtt)==s_rtt && !path){
                                if((sub_tp->srtt_us>>3)==s_rtt && !path){
                        		s_rtt=(sub_tp->srtt_us>>3);
                        		//s_rtt=(sub_tp->sandy_rtt);
					s_tp=sub_tp;
					...
					l_rtt=(sub_tp->srtt_us>>3);
					//l_rtt=(sub_tp->sandy_rtt);
					l_tp=sub_tp;
					l_sk=sk_it;
					path=0;
				}
			}
			if(s_tp==NULL || l_tp==NULL)
				printk("\nMan your sub flow tp NULL\n");
			//Measurre RTT difference
			rttdiff=jiffies_to_msecs(usecs_to_jiffies(s_rtt))-jiffies_to_msecs(usecs_to_jiffies(l_rtt));
			if(rttdiff<0)
				rttdiff*=-1;

			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                                sub_tp=tcp_sk(sk_it);
				sub_tp->sandy_rttdiff=rttdiff;
			}	



			//Save rtt of each other paths
			if (s_tp!=NULL && l_tp!=NULL)
				s_tp->sandy_ortt=l_tp->srtt_us;
				//s_tp->sandy_ortt=l_tp->sandy_rtt;
			if (s_tp!=NULL && l_tp!=NULL)
				l_tp->sandy_ortt=s_tp->srtt_us;
				//l_tp->sandy_ortt=s_tp->sandy_rtt;

			//Find ofo difference
			ofo_diff=skb_queue_len(&meta_tp->out_of_order_queue)-prev_ofo;		
			if(ofo_diff>0)
				ofo_sum+=ofo_diff;
			else 
				ofo_count++;
			...
			path=0;
			//Bytes of smallest RTT path:
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                                sub_tp=tcp_sk(sk_it);
				if((sub_tp->srtt_us>>3)==s_rtt && !path){
				//if((sub_tp->sandy_rtt)==s_rtt && !path){
					s_total=sub_tp->bytes_received;
					path=1;
				}
				else{
					l_total=sub_tp->bytes_received;
					path=0;
				}
			}
			...
			if(prev_time==0)
				prev_time=jiffies_to_msecs(tcp_jiffies32);
			if(prev_bytes==0)
				prev_bytes=meta_tp->bytes_received;
			if(start_time==0)
				start_time=jiffies_to_msecs(tcp_jiffies32);
			if(tcp_space(meta_sk)<=meta_tp->rcv_wnd)
				ratio=(tcp_space(meta_sk)*100)/meta_sk->sk_rcvbuf;
			else
				ratio=(meta_tp->rcv_wnd*100)/meta_sk->sk_rcvbuf;
			printk("\nRatio will be %u %u\n",meta_sk->sk_rcvbuf,ratio);
			if(prev_time-start_time>=1000){
				
				total_download=prev_bytes-start_bytes;
				//total_download<<=3;
				sub_total_download=sub_prev_bytes-sub_start_bytes;
				//sub_total_download<<=3;
				//s_total_download=(s_prev_bytes-s_start_bytes)+(ofo_sum*meta_tp->advmss);
				s_total_download=(s_prev_bytes-s_start_bytes);
				//s_total_download<<=3;
				l_total_download=l_prev_bytes-l_start_bytes;
				//l_total_download<<=3;
				if(s_rtt!=l_rtt){
					if(s_rtt<=l_rtt && s_total_download>=l_total_download){
						s_total_download+=prev_ofo*meta_tp->advmss;
						if(s_tp!=NULL) s_tp->sandy_best=1;
					}
					else if(s_rtt<=l_rtt && s_total_download<l_total_download){
						l_total_download+=prev_ofo*meta_tp->advmss;
						if(l_tp!=NULL) l_tp->sandy_best=1;
					}
					else if(s_rtt>l_rtt && s_total_download>=l_total_download){
						s_total_download+=prev_ofo*meta_tp->advmss;
						if(s_tp!=NULL) s_tp->sandy_best=1;
					...
					if(s_total_download<l_total_download){
						l_total_download+=prev_ofo*meta_tp->advmss;
						if(l_tp!=NULL) l_tp->sandy_best=1;
					}
					else{
				...
				mthr=total_download/total_time;
				emthr=sub_total_download/total_time;
				s_thr=s_total_download/total_time;
				l_thr=l_total_download/total_time;



				if(s_thr>l_thr && s_thr>0 && l_thr>0){
					tbest=s_thr;
				}
				else if(s_thr>0 && l_thr>0){
					tbest=l_thr;
				}
				printk("\nCondition to trigger value %u %u  %u\n",emthr,tbest,(((tbest*ratio)/100)+tbest));
				//if(emthr>=(((tbest*ratio)/100)+tbest) && emthr>0 && tbest>0 && s_total_download >0 && l_total_download>0){
				if(emthr>=tbest && emthr>0 && tbest>0 && s_total_download >0 && l_total_download>0){
					condition=true;
				}
				//else if(emthr>0 && prev_ofo>0 && emthr<(((tbest*ratio)/100)+tbest) && s_total_download >0 && l_total_download>0){
				else if(emthr>0 && prev_ofo>0 && emthr<tbest && s_total_download >0 && l_total_download>0){
					condition=false;
					/*if(s_tp&&l_tp){
						if(s_tp->sandy_best==1 && l_tp->sandy_best==0 && l_tp->sandy_block==0 && l_tp->sandy_check==0 && s_tp->sandy_block==0 && s_tp->sandy_check==0){
							l_tp->sandy_block=1;
							l_tp->sandy_block_time=jiffies_to_msecs(tcp_jiffies32);
							l_tp->sandy_rttdiff=jiffies_to_msecs(usecs_to_jiffies(l_tp->srtt_us>>3))-jiffies_to_msecs(usecs_to_jiffies(s_tp->srtt_us>>3));
							//l_tp->sandy_rttdiff=l_tp->sandy_rtt-s_tp->sandy_rtt;
							l_tp->sandy_brtt=l_tp->sandy_rttdiff;//This has to adopt
							//l_tp->sandy_brtt=(l_tp->sandy_rttdiff*ratio)/100+l_tp->sandy_rttdiff;
							if(l_tp->sandy_rttdiff<0)
								l_tp->sandy_rttdiff*=-1;
							printk("\n%u  MP_CHECK blocking the path %pI4 at RTT diff: %d ack diff %u\n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)l_tp)->inet_saddr,l_tp->sandy_rttdiff,l_tp->sandy_rtt-s_tp->sandy_rtt);
						}
						else  if(s_tp->sandy_best==0 && l_tp->sandy_best==1 && s_tp->sandy_block==0 && s_tp->sandy_check==0 && l_tp->sandy_block==0 && l_tp->sandy_check==0){
							s_tp->sandy_block=1;	
							s_tp->sandy_block_time=jiffies_to_msecs(tcp_jiffies32);
							s_tp->sandy_rttdiff=jiffies_to_msecs(usecs_to_jiffies(l_tp->srtt_us>>3))-jiffies_to_msecs(usecs_to_jiffies(s_tp->srtt_us>>3));
							//s_tp->sandy_rttdiff=l_tp->sandy_rtt-s_tp->sandy_rtt;
							s_tp->sandy_brtt=s_tp->sandy_rttdiff;//This has to adopt
							//s_tp->sandy_brtt=(s_tp->sandy_rttdiff*ratio)/100+s_tp->sandy_rttdiff;
                                                        if(s_tp->sandy_rttdiff<0)
                                                                s_tp->sandy_rttdiff*=-1;
							printk("\n%u  MP_CHECK blocking the path %pI4 at RTT diff : %d\n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)s_tp)->inet_saddr,s_tp->sandy_rttdiff);
						}			

					}*/
				}/*else{ //after blocking the path
					if(l_tp->sandy_block==1 && ((jiffies_to_msecs(tcp_jiffies32)-l_tp->sandy_block_time)>900) && l_sk!=NULL && l_tp->sandy_check==0 && s_tp->sandy_block==0 && s_tp->sandy_check==0){
						printk("\n%u MP_CHECK Active RTT check for blocked %pI4 \n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)l_tp)->inet_saddr);
						l_tp->sandy_check=1;
						l_tp->sandy_block_time=jiffies_to_msecs(tcp_jiffies32);
						tcp_send_ack(l_sk); 			
					}
					else if(s_tp->sandy_block=1 && s_sk!=NULL && ((jiffies_to_msecs(tcp_jiffies32)-s_tp->sandy_block_time)> 900) && s_tp->sandy_check==0 && l_tp->sandy_block==0 && l_tp->sandy_check==0){
						printk("\n %u MP_CHECK Active RTT check for blocked %pI4 \n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)s_tp)->inet_saddr);
						s_tp->sandy_check=1;
						s_tp->sandy_block_time=jiffies_to_msecs(tcp_jiffies32);
						tcp_send_ack(s_sk);					
					}
				...
				//printk("\nsandy:MPTCP total bits %ld small RTT %ld  latger RTT %ld total time %ld and throughput in bps %ld smallest RTT %ld and larger RTT %ld receive window %u and receive space %u OFO length %u condition %d\n",sub_total_download,s_total_download,l_total_download,total_time,emthr,s_thr,l_thr,meta_tp->rcv_wnd,tcp_space(meta_sk),skb_queue_len(&meta_tp->out_of_order_queue),condition);
				if(sub_total_download>0 && s_total_download >0 && l_total_download>0)
					printk("\n%u sandy:MPTCP total bits %ld sRTT %ld  lRTT %ld total time %ld and t_mptcp %ld t_s %ld and t_l %ld r_w %u and r_s %u OFO  %u condition %d s_rtt %u l_rtt %u\n",jiffies_to_msecs(tcp_jiffies32),sub_total_download,s_total_download,l_total_download,total_time,emthr,s_thr,l_thr,meta_tp->rcv_wnd,tcp_space(meta_sk),prev_ofo,condition,jiffies_to_msecs(usecs_to_jiffies((s_tp->srtt_us>>3))),jiffies_to_msecs(usecs_to_jiffies((l_tp->srtt_us>>3))));

				/*path=0;
                        	mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
                                	sub_tp=tcp_sk(sk_it);
					if((sub_tp->srtt_us>>3)==s_rtt && !path){
						printk("\n%u %pI4 rtt %u artt %d last received %u and last sent %u \n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)sub_tp)->inet_saddr,jiffies_to_msecs(usecs_to_jiffies((sub_tp->srtt_us>>3))),sub_tp->sandy_rtt,jiffies_to_msecs(sub_tp->rcv_tstamp),sub_tp->sandy_astime);
						path=1;
				...
				start_time=jiffies_to_msecs(tcp_jiffies32);
				start_bytes=meta_tp->bytes_received;
				sub_start_bytes=subtotal;
				s_start_bytes=s_total;
				l_start_bytes=l_total;
				//ofo_sum=0;
			}
			prev_time=jiffies_to_msecs(tcp_jiffies32);
			prev_bytes=meta_tp->bytes_received;
			sub_prev_bytes=subtotal;
			s_prev_bytes=s_total;
			l_prev_bytes=l_total;
			prev_ofo=skb_queue_len(&meta_tp->out_of_order_queue);
        	}
		printk("\nTotal OFO length : %u Total OFO occupied memory %u Count %d\n",ofo_sum,ofo_sum*meta_tp->advmss,ofo_count);
...
void mptcp_init_buffer_space(struct sock *sk)
{
	...
	if (space > meta_sk->sk_rcvbuf) {
		//printk("\n********************Doubing the size for meta socket******************************\n");
		//printk("\n******************** Before doubling  meta_tp->window_clamp %d meta_tp->rcv_ssthresh %d meta_sk->sk_rcvbuf %d *******************\n",meta_tp->window_clamp,meta_tp->rcv_ssthresh,meta_sk->sk_rcvbuf);
		meta_tp->window_clamp += tp->window_clamp;
		meta_tp->rcv_ssthresh += tp->rcv_ssthresh;
		meta_sk->sk_rcvbuf = space;
		//printk(" \n**************After doubling  meta_tp->window_clamp %d meta_tp->rcv_ssthresh %d meta_sk->sk_rcvbuf %d ****************\n",meta_tp->window_clamp,meta_tp->rcv_ssthresh,meta_sk->sk_rcvbuf);
	}
