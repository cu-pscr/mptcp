...
#include <net/sock.h>
__u32 sandy_hm_rtt=0;
__u32 sandy_hm_cwnd=0;
		     ...
		     int push_one, gfp_t gfp)
{
	...
	printk("\n%u MPTCP level send window %u\n",jiffies_to_msecs(tcp_jiffies32),meta_tp->snd_wnd);
...
u32 __mptcp_select_window(struct sock *sk)
{
	...
	printk("\n%s free_Space %d full_space %d mss %d and window %d ssthresh %d window clamp %d %d rx_opt.rcv_wscale %d\n",__func__,free_space,full_space,mss,meta_tp->rcv_wnd,meta_tp->rcv_ssthresh,meta_tp->window_clamp,meta_tp->rx_opt.rcv_wscale);
	if (mss > full_space)
		mss = full_space;
	...
	//printk("sandy:MPTCP level: total buffer %d and receiver buffer %d and total queued %d receiver queue length %d and OFO queue legth %d\n",meta_sk->sk_rcvbuf,window,(meta_tp->rcv_nxt-meta_tp->copied_seq),skb_queue_len(&meta_sk->sk_receive_queue),skb_queue_len(&meta_tp->out_of_order_queue));

	if(((meta_tp->srtt_us>>3))>sandy_hm_rtt)
                sandy_hm_rtt=(meta_tp->srtt_us>>3);
        if(meta_tp->snd_cwnd>sandy_hm_cwnd)
                sandy_hm_cwnd=meta_tp->snd_cwnd;
        //printk("sandy:  mptcp_snd_nxt %u mptcp_snd_una %u mptcp_snd_cwnd %u mptcp_snd_wnd %u mptcp_rcv_wnd %u mptcp_RTT %u mptcp_MSS %u mptcp_packets in air %u highest CWND %u and highest RTT %u\n",meta_tp->snd_nxt,meta_tp->snd_una,meta_tp->snd_cwnd,meta_tp->snd_wnd,meta_tp->rcv_wnd,(meta_tp->srtt_us>>3),tcp_current_mss(meta_sk),meta_tp->packets_out,sandy_hm_cwnd,sandy_hm_rtt);
	printk("\n%u sandy: MPTCP level receiver window  %u\n",jiffies_to_msecs(tcp_jiffies32),window);
	return window;
}
...
int mptcp_retransmit_skb(struct sock *meta_sk, struct sk_buff *skb)
{
	...
	int err = -1;
	WARN_ON(TCP_SKB_CB(skb)->sacked);
	//printk("\nMPTCP meta-level Retranmistting the packet\n");
