...
ktime_t sandy_time;
unsigned long long int ofo_count=0;
		...
		if(meta_tp && meta_sk){
			...
			mptcp_for_each_sk(meta_tp->mpcb, sk_it) {
				...
				if((sub_tp->srtt_us>>3)==s_rtt && !path){
					s_total=sub_tp->bytes_received+((skb_queue_len(&meta_tp->out_of_order_queue)+1)*meta_tp->advmss);
					path=1;
				}
				else{
			...
			if(prev_time-start_time>=1000 ){
				...
				if(s_thr>l_thr && s_thr>0 && l_thr>0)
					...
					tbest=l_thr;
				if(emthr>=(tbest*0.01)+tbest && emthr>0 && tbest>0)
					condition=true;
				else if(emthr>0 && tbest>0)
					condition=false;
					
				printk("\nsandy:MPTCP total bits %ld small RTT %ld  latger RTT %ld total time %ld and throughput in bps %ld smallest RTT %ld and larger RTT %ld receive window %u and receive space %u condition %d\n",sub_total_download,s_total_download,l_total_download,total_time,emthr,s_thr,l_thr,meta_tp->rcv_wnd,tcp_space(meta_sk),condition);
