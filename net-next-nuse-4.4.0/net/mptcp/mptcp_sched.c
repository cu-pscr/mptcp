...
bool mptcp_is_def_unavailable(struct sock *sk)
{
	...
	int window=0;
	//printk("sandy:%sSender here\n",__func__);
	//sandy code below to compute ipaddress
        /*if(inet!=NULL){
                ipaddr=__be32_to_cpu(inet->inet_daddr);
		//printk("sandy:%s the obtained IP is %llu\n",__func__,ipaddr);
                while(ipaddr>2147483647){
                        ipaddr-=2147483647;
                }
        	if(ipaddr==167837953){//hard coded ip=10.1.1.1
                	//printk("**************************************sandy:%sfast path and its send window  %d**********************************\n",__func__,tp->rcv_wnd);
			window=tp->rcv_wnd;
        	}
        }else{
		//printk("sandy: %s cannot obatain ip address",__func__);
		window=-1;
	}*/
				   ...
				   bool zero_wnd_test)
{
	...
	mptcp_for_each_tp(tcp_sk(meta_sk)->mpcb, tp_it) {
		if(tp_it->snd_cwnd>tp_it->sandy_hs_cwnd)
			tp_it->sandy_hs_cwnd=tp_it->snd_cwnd;
		if((tp_it->snd_wnd/tp_it->advmss)>tp_it->sandy_hs_snd)
			tp_it->sandy_hs_snd=(tp_it->snd_wnd/tp_it->advmss);
	}
	...
	//printk("\n** sandy this is your default scheduler **\n");
	return sk;
}
	...
	mptcp_for_each_tp(tp->mpcb, tp_it) {
		if (tp_it != tp &&
			...
			if (tp->srtt_us < tp_it->srtt_us && inet_csk((struct sock *)tp_it)->icsk_ca_state == TCP_CA_Open) {
				...
				printk("sandy:%s penalization is enabled fpr %pI4 %u before cwnd: %u and after cwnd %u\n",__func__,&((struct inet_sock *)tp_it)->inet_daddr,&((struct inet_sock *)tp_it)->inet_dport,prior_cwnd,(tp_it->snd_cwnd>>1U));
					  ...
					  unsigned int *limit)
{
	...
	int space_left=0;
	if(skb!=NULL && !*reinject){
		space_left=tcp_wnd_end(tcp_sk(meta_sk))-TCP_SKB_CB(skb)->end_seq;
		printk("\nsandy:MPTCP level:send window %u %u space left %d\n",jiffies_to_msecs(tcp_jiffies32),tcp_sk(meta_sk)->snd_wnd,space_left);
	}
...
void mptcp_init_scheduler(struct mptcp_cb *mpcb)
{
	...
	//printk("sandy:%s MPTCP default scheduler\n",__func__);
	rcu_read_lock();
