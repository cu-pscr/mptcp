...
new_segment:
			 ...
			 */
			if (((mptcp(tp) && !tp->mpcb->dss_csum) || !mptcp(tp)) &&(mptcp(tp) ||sk_check_csum_caps(sk)))
				skb->ip_summed = CHECKSUM_PARTIAL;
...
do_prequeue:
	...
	found_ok_skb:
		...
		if(is_meta_sk(sk)) {
			...
			mptcp_queue_probe_log_hook(sk,0, mptcp_meta_tp(tcp_sk(sk)), skb_log, 1);
		}
		*seq += used;
		copied += used;
		len -= used;
