...
static int __tcp_grow_window(const struct sock *sk, const struct sk_buff *skb)
{
	...
	while (tp->rcv_ssthresh <= window) {
		if (truesize <= skb->len){
			//printk("\n%s I am rturning %d and inet_csk(sk)->icsk_ack.rcv_mss =%d\n",__func__,2 * inet_csk(sk)->icsk_ack.rcv_mss,inet_csk(sk)->icsk_ack.rcv_mss);
...
static void tcp_grow_window(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (meta_tp->rcv_ssthresh < meta_tp->window_clamp &&
		...
		if (incr) {
			//printk("\n%s returned value  %d and  2 * skb->len=%d and seq diff %d and true length %d\n",__func__,incr,(2*skb->len),TCP_SKB_CB(skb)->end_seq-TCP_SKB_CB(skb)->seq,skb->truesize);
			       ...
			       int fast_rexmit, int flag)
{
	...
	//printk("\n%s before reduction in cwnd %u\n",__func__,tp->snd_cwnd);
	tp->snd_cwnd = tcp_packets_in_flight(tp) + sndcnt;
	//printk("\n%s after reduction in cwnd %u\n",__func__,tp->snd_cwnd);
}
				 ...
				 u32 ack_seq)
{
	...
	printk("\n%u sandy:%sThe received window is for %pI4 is %d\n",jiffies_to_msecs(tcp_jiffies32),__func__,&((struct inet_sock *)tp)->inet_daddr,nwin);
	...
	if (tcp_may_update_window(tp, ack, ack_seq, nwin)) {
		...
		//printk("sandy:%sThe window value is set to %d\n",__func__,tp->snd_wnd);
	}
...
static int tcp_ack(struct sock *sk, struct sk_buff *skb, int flag)
{
	...
	if (tcp_ack_is_dubious(sk, flag)) {
		//printk("DUP received for the tcp %pI4 and cwnd %u and ACK number %u \n",&((struct inet_sock *)tp)->inet_daddr,tp->snd_cwnd,TCP_SKB_CB(skb)->ack_seq);
...
static void tcp_ofo_queue(struct sock *sk)
{
	...
	while ((skb = skb_peek(&tp->out_of_order_queue)) != NULL) {
		...
		if (is_meta_sk(sk)) {
            		mptcp_queue_probe_log_hook(sk,1, mptcp_meta_tp(tcp_sk(sk)), skb, 1);
            		mptcp_queue_probe_log_hook(sk,0, mptcp_meta_tp(tcp_sk(sk)), skb, 0);
        	}	
		if (!eaten)
...
slow_path:
	//sandy:printing the OFO queue length and received bytes
	/*if(is_meta_sk(sk) && mptcp_meta_tp(tcp_sk(sk))){
		printk("\nMPTCP level bytes received %lu and OFO %lu\n",mptcp_meta_tp(tcp_sk(sk))->bytes_received,skb_queue_len(&(mptcp_meta_tp(tcp_sk(sk)))->out_of_order_queue));
	}*/
	if( tp->sandy_flag &&  tp->sandy_astime >0 && TCP_SKB_CB(skb)->seq>=tp->sandy_ack){
		tp->sandy_rtt=jiffies_to_msecs(tcp_time_stamp)-tp->sandy_astime;
		printk("\nMy RTT is %u and sandy RTT %d seq: %u\n",jiffies_to_msecs(usecs_to_jiffies(tp->srtt_us>>3)),tp->sandy_rtt,TCP_SKB_CB(skb)->seq);
		tp->sandy_flag=0;
	}
...
step5:
	...
	/*sandy:Check for the updated RTT
        */
        if(tp->sandy_block==1 && tp->sandy_check==1){
		rtt_diff=jiffies_to_msecs(usecs_to_jiffies(tp->srtt_us>>3))-jiffies_to_msecs(usecs_to_jiffies(tp->sandy_ortt>>3));
		if(rtt_diff<0)
			rtt_diff*=-1;
		ack_rtt=jiffies_to_msecs(tp->rcv_tstamp)-tp->sandy_astime;
		if(ack_rtt<0)
			ack_rtt*=-1;
		ack_diff=ack_rtt-jiffies_to_msecs(usecs_to_jiffies(tp->sandy_ortt>>3));
		if(ack_diff<0)
			ack_diff*=-1;
		if(ack_diff> rtt_diff)
			diff=ack_diff;
		else diff=rtt_diff;
		if(diff<0)
			diff*=-1;	
		printk ("\nMP_CHECK trying to unblock the path time stamp of sent %u time stamp of received %u\n",tp->sandy_astime,jiffies_to_msecs(tp->rcv_tstamp));
		printk("\n%u MP_CHECK: ip %pI4 my RTT %d  %d and other path RTT %d  diff %d and limit %u\n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)tp)->inet_daddr,jiffies_to_msecs(usecs_to_jiffies(tp->srtt_us>>3)),tp->sandy_rtt,jiffies_to_msecs(usecs_to_jiffies(tp->sandy_ortt>>3)),diff,tp->sandy_brtt);
                if(diff<=tp->sandy_brtt){
			printk("\n%u MP_CHECK Releasing the blocked ip %pI4 diff %u and limit %u\n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)tp)->inet_saddr,diff,tp->sandy_brtt);
			tp->sandy_block=0;
			tp->sandy_check=0;
		}else{
			printk("\n%u MP_CHECK condition to release the blocked ip %pI4 failed diff %u and limit %u\n",jiffies_to_msecs(tcp_jiffies32),&((struct inet_sock *)tp)->inet_saddr,diff,tp->sandy_brtt);
			tp->sandy_check=0;
		}
	}
					 ...
					 const struct tcphdr *th)
{
	...
	if (th->ack) {
		 ...
		 * never scaled.
		 */
		tp->snd_wnd = ntohs(th->window);//sandy window received
...
discard:
	...
	if (th->syn) {
		 ...
		 * never scaled.
		 */
		tp->snd_wnd    = ntohs(th->window);//sandy receive window
	...
	__releases(&sk->sk_lock.slock)
{
	...
	case TCP_SYN_RECV:
		...
		tp->snd_wnd = ntohs(th->window) << tp->rx_opt.snd_wscale;//sandy received window
		tcp_init_wl(tp, TCP_SKB_CB(skb)->seq);
